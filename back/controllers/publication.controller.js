...
exports.getAllPublications = (req, res) => {
  const getAllPublicationsRequest =
    'SELECT user.id AS userId, lastName, firstName, avatarUrl, posts.id AS postId, posts.postContent, posts.imageUrl, DATE_FORMAT(posts.post_date, "%d/%m/%Y") AS post_date, posts.comment_count FROM user JOIN posts ON posts.userId = user.id WHERE posts.active = 1 ORDER BY posts.post_date DESC';
